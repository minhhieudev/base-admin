...
export default {
  ...
  methods: {
    ...
    getPrice(pack) {
      if (this.$store.getters.user && this.$store.getters.user.is_partner && pack.vip_price) {
    ...
    handleCopyStr(str) {
      ...
      if (selected) {                                 
        ...
        this.$notify({ type: 'success', message: 'Đã sao chép vào clipboard.' })
      }
    },
    formatPrice (price) {
      price = Number(price)
    ...
    acl (path) {
      ...
      path = path.split('/')
      path = path.map(p => { return snakeCase(p) }).join('/')
